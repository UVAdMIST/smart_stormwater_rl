...
from smart_stormwater_rl.RL_DQN.DQN import DeepQAgent, build_network, epsi_greedy, throttle_valves
...
epi_start, epi_end = 0.3, 0.05
epsilon_value = np.linspace(epi_start, epi_end, num_episodes + 10)
throttle = 0.05
...
model_dir = "smart_stormwater_rl/RL_DQN/saved_models"
reward_dir = "smart_stormwater_rl/RL_DQN/saved_model_rewards"
...
# Initialize Action value function, two networks: Q-network and Q-learning targets
# model = target = build_network(len(input_states), 2 ** len(temp_valve), 2, 100, 'relu', 0.0)
...
while episode < num_episodes:  # loop through all episodes
    ...
    while step_count <= num_steps - 1:  # loop through all steps in the simulation
        ...
        if step_count >= num_steps:
            ...
            # # calculate reward for previous states
            # reward = reward_function(temp_height, temp_flood)
            # reward_sim.append(reward)
            # # print("reward: ", reward)

            # old_input_states = np.copy(input_states)
            # old_action = action

            # construct current system states as DQN inputs
            ...
            # Policy
            action = epsi_greedy(len(action_space), q_values, epsilon_value[episode])
            # print("action: ", action)

            # Implement Action
            throttle_valves(valve_list, temp_valve, action_space, action, throttle)

            # Run step with selected action
...
save_state_name = "DQN_" + str(num_episodes) + "_states_" + rwd
save_action_name = "DQN_" + str(num_episodes) + "_actions_" + rwd
...
plt.tight_layout()
# plt.show()
plt.savefig("smart_stormwater_rl/RL_DQN/plots/dqn_model_results_" + str(best_episode) + rwd + ".png", dpi=300)
plt.close()
...
plt.xlabel("episode")
plt.tight_layout()
plt.savefig("smart_stormwater_rl/RL_DQN/plots/dqn_model_rewards_" + str(num_episodes) + rwd + ".png", dpi=300)
plt.close()
